...
import { Notification, Severity, Topic, NotificationStatus, NotificationUpdatePayload, User } from '../../types';
...
export const RecentNotifications: React.FC<RecentNotificationsProps> = ({ 
    ...
    const filteredNotifications = useMemo(() => {
        ...
        if (timeFilter !== 'all') {
            ...
            notifs = notifs.filter(n => n.timestamp && new Date(n.timestamp) > filterDate);
        }

        if (searchTerm) {
            const lowercasedFilter = searchTerm.toLowerCase();
            notifs = notifs.filter(n =>
                (n.title && n.title.toLowerCase().includes(lowercasedFilter)) ||
                ...
                <div className="p-2 space-y-2">
                    {filteredNotifications.length === 0 ? (
                        <div className="text-center py-16 text-muted-foreground">
                            ...
                            <p className="text-sm mt-1">When a new alert is triggered, it will appear here.</p>
                        </div>
                    ) : filteredNotifications.map(n => {
                        const isProcessing = processingIds.has(n.id);
                        const severityInfo = SEVERITY_INFO[n.severity] || { color: 'text-gray-500', icon: 'help-circle' };
                        const statusInfo = STATUS_INFO[n.status] || { bg: 'bg-gray-100', text: 'text-gray-800', icon: 'help-circle' };
                                ...
                                >
                                    <div className="grid grid-cols-[2rem,1fr,auto] gap-4 items-start">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="flex items-center gap-3 mt-3">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full ${statusInfo.bg} ${statusInfo.text} capitalize`}>
                                                    ...
                                                    {isProcessing ? 'Processing...' : (n.status || 'unknown')}
                                                </span>
                                                <button 
                                        ...
                                        <div className="flex items-center gap-2">
                                             <p className="text-xs text-muted-foreground flex-shrink-0">{n.timestamp ? new Date(n.timestamp).toLocaleString([], {
                                                dateStyle: 'short',
                                                timeStyle: 'short',
                                            }) : 'No date'}</p>
            ...
            {infoModalNotification && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setInfoModalNotification(null)}>
                    <div className="bg-card text-card-foreground p-6 rounded-lg shadow-xl max-w-md w-full" onClick={e => e.stopPropagation()}>
                        ...
                        {(() => {
                            const safeTopics = Array.isArray(topics) ? topics : [];
                            const topic = safeTopics.find(t => t.id === infoModalNotification.topic_id);
                            return (
                                <div>
