...
interface AddSiteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSiteAdded: (site: Omit<MonitoredSite, 'id' | 'created_at' | 'updated_at'>) => void;
}
...
export const AddSiteModal: React.FC<AddSiteModalProps> = ({ isOpen, onClose, onSiteAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`https://api.first.org/data/v1/countries?q=${selectedCountry.name}&limit=1`);
      const data = await response.json();
      const { lat, long } = data.data[Object.keys(data.data)[0]];
